...
- **Last round's edits are highlighted by default**: opening the plan after
  a planning turn shows the whole plan with additions tinted **green** and
  removals tinted **red** (with strikethrough) right where they occur.
  Single-line edits are merged inline, and the title shows a `+N −M`
  summary. There are no `+`/`-` gutter markers, so copy-paste of the plan
  stays clean. The first planning round (no prior baseline) and unchanged
  rounds render normally.
